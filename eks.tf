...
resource "aws_eks_node_group" "worker_obl" {                            # Este recurso nos permite crear los Workers que tendra el cluster.
  ...
  subnet_ids      = [aws_subnet.publica-1a.id,aws_subnet.publica-1b.id] # Se le asigna que subnets utilizaran los Workers
  instance_types = ["t3.medium"]                                        # Se indica el tipo de instancia que utilizaran los Workers
